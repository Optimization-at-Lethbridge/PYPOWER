...
#from .util import  checkHHLCompatibility
...
def newtonpf(Ybus, Sbus, V0, ref, pv, pq, ppopt=None):
    ...
    @autor: Ray Zimmerman (PSERC Cornell)
    """
    ## default arguments
    print('Updated newtonpf....................................')
    if ppopt is None:
        ppopt = ppoption()
    ...
    while (not converged and i < max_it):
        ...
        while True:
            ...
            J_dense = J.toarray()
            #case 3
            #vqls_solver = VQLSSolver(J_dense, F, num_layers=3, max_iterations=250)

            #case 4gs
            #vqls_solver = VQLSSolver(J_dense, F, num_layers=3, conv_tol=1e-17, max_iterations=220)

            #case 5
            #vqls_solver = VQLSSolver(J_dense, F, conv_tol=1e-17, max_iterations=180)

            #case 6ww
            #vqls_solver = VQLSSolver(J_dense, F, conv_tol=1e-17)

            #case 9
            vqls_solver = VQLSSolver(J_dense, F, conv_tol=1e-17, max_iterations=150)
            ...
            ## evaluate the potential new V and the new mismatch
            Va_new = Va.copy()
            Vm_new = Vm.copy()

            if npv:
                Va_new[pv] = Va[pv] + dx[j1:j2]
            if npq:
                Va_new[pq] = Va[pq] + dx[j3:j4]
                Vm_new[pq] = Vm[pq] + dx[j5:j6]

            V_new = Vm_new * exp(1j * Va_new)
            mis_new = V_new * conj(Ybus * V_new) - Sbus
            F_new = r_[  mis_new[pv].real,
                         mis_new[pq].real,
                         mis_new[pq].imag  ]

            normF_new = linalg.norm(F_new, Inf)

            if normF_new < normF:
                Va, Vm, V = Va_new, Vm_new, V_new
                F, normF = F_new, normF_new
                break
            else:
                if verbose > 1:
                    sys.stdout.write('\nRecalculating dx since normF_new >= normF')
